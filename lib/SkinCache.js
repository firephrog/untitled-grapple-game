...
class SkinCache {
  ...
  getSkin(skinId, type = 'player') {
    ...
    try {
      ...
      this._misses++;
      console.log(`[SkinCache] Loaded: ${cacheKey} (${(buffer.length / 1024).toFixed(1)}KB)`);
      return buffer;
    } catch (err) {
  ...
  preloadSkins(skinIds, type = 'player') {
    ...
    console.log(`[SkinCache] Preloaded ${loaded}/${skinIds.length} ${type} skins`);
  }
  ...
  unloadSkin(skinId, type = 'player') {
    ...
    if (this._cache.has(cacheKey)) {
      ...
      console.log(`[SkinCache] Unloaded: ${cacheKey} (freed ${(buffer.length / 1024).toFixed(1)}KB)`);
    }
  }
  ...
  clear() {
    ...
    console.log(`[SkinCache] Cleared all skins (freed ${(memBefore - memAfter) / 1024 / 1024}.toFixed(1)}MB)`);
  }
